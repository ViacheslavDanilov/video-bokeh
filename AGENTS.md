...
- Two `.env.example` templates are checked in: `backend/.env.example` for the backend process (`VIDEO_BOKEH_LIBRARY`, `VIDEO_BOKEH_DATA_ROOT`, and `CORS_ORIGINS`, which nothing reads yet), and the root `.env.example` for `docker compose` (`VIDEO_BOKEH_DATA_ROOT`, `VIDEO_BOKEH_LIBRARY`). Nothing auto-loads the backend one — there is no dotenv in the backend, so those are exported or passed on the command line.
